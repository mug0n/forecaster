--[[
    sources:
        https://github.com/LandSandBoat/server/blob/base/src/common/vana_time.h
        https://github.com/LandSandBoat/server/blob/base/src/common/vana_time.cpp
...
--[[
    Converts day of the week number to text value

    @param  {number} dayID          Day of the week number, i.e. 1
    @return {string}                Day of the week text, i.e. "Earthsday", or nil upon failure
]]--
function VanaUtils.dayValueToText(dayID)
    return VanaUtils.Weekdays[dayID] or nil
end

--[[
    Get the zone data from its id

    @param  {number} zoneID         Zone id
    @return {number, string}        Zone ID followed by zone name, or nil upon failure
]]--
function VanaUtils.getZoneByID(zoneID)
    -- otherwise assuming input is the id
...
--[[
    Get the zone data from user input (either text or zone id)

    @param  {mixed} input           Text or number input
    @return {number, string}        Zone ID followed by zone name, or nil upon failure
]]--
function VanaUtils.getZoneByInput(input)
    ...
    if (charPos ~= nil) then
        ...
        return VanaUtils.getZoneByID(input)
    end
...
--[[
    Returns SE epoch in earth seconds.

    @return {number}                SE epoch in earth seconds
]]--
function VanaUtils.getVanaEpoch()
    return os.time() - 1009810800
end

--[[
    Converts from SE epoch to vanadiel minutes and add 886 vana years.

    @return {number}                Current Vanadiel time
]]--
function VanaUtils.getVanaTime()
    return math.floor((VanaUtils.getVanaEpoch() / 60 * 25) + 459302400)
end

--[[
    Make VanaTime human readable

    @param  {number} vanaTime       Vanadiel time value, defaults to current if omitted
    @return {string, string}        Vanadiel date string followed by Day of the week string
]]--
function VanaUtils.getVanaDate(vanaTime)
    -- get current vanadiel time if no value is set
    vanaTime = vanaTime or VanaUtils.getVanaTime()

    local vanaYear = math.floor(vanaTime / 518400)
    local vanaMonth = math.floor((vanaTime / 43200) % 12) + 1
    local vanaDay = math.floor((vanaTime / 1440) % 30) + 1
    local vanaElementDay = math.floor((vanaTime % 11520) / 1440)

    -- return properly formatted date string and day of the week
    return string.format("%04u/%02u/%02u", vanaYear, vanaMonth, vanaDay), VanaUtils.dayValueToText(vanaElementDay)
end
