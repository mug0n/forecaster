...
local function getCharPos(str, charCode)
...
function VanaUtils.dayValueToText(value)
    return VanaUtils.Weekdays[value] or false
end


function VanaUtils.getZoneFromInput(input)
    -- input might be an id or an auto-translate string
    local charPos = getCharPos(input, 253)
    if (charPos ~= nil) then
        ...
        return VanaUtils.getZoneFromID(input)
    end

    return nil
end

function VanaUtils.getZoneFromID(zoneID)
...
--[[
    Make VanaTime useful and readable

    @param  {number} vanaTime       Vanadiel time value
    @return {string, number}        Vanadiel date string followed by Day of the week number
]]--
